...
One-line "when to use" summary. This becomes the search surface in the CDR Index table for the `team.discover` command. Derive from the `### Context` and `### Decision` sections — e.g., "SQL injection prevention patterns for all languages" or "Java Google style guide conventions for new projects".
